...
namespace BTCPayServer
{
    public partial class BTCPayNetworkProvider
    {
        ...
        {
#if !ALTCOINS
            var onlyBTC = selectedChains.Count == 1 && selectedChains.First() == "BTC";
            if (!onlyBTC)
            ...
            {
                if (selectedChains.Contains(network.CryptoCode.ToUpperInvariant()))
                {
            ...
            logs.Configuration.LogInformation(
                "Supported chains: " + String.Join(',', GetAll().Select(n => n.CryptoCode).ToArray()));
        }
